...
export default async function ServiceCategoriesPage() {
  const ctx = await requireScreen('service_categories', 'view');
  const repo = new ServiceCategoriesRepository(ctx.supabase);
  const perms = new PermissionsService(ctx.supabase);
  const [rows, total, canEdit] = await Promise.all([
    repo.list({ limit: 200 }),
    repo.count(),
    perms.can('service_categories', 'edit'),
  ]);
  const t = await getTranslations();
  return (
    <SimpleMasterClient
      title={t('masters.serviceCategories')}
      rows={rows}
      total={total}
      canAdd={canEdit}
      canEdit={canEdit}
      apiPath="/api/masters/service-categories"
      defaultValues={{ code: '', name_ar: '', name_en: '', display_order: 0, is_active: true }}
      fields={[
        { name: 'code', label: t('common.code'), required: true, dir: 'ltr', placeholder: 'MAINT_SVC' },
